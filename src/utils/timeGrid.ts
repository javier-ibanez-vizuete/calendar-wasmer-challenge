...
export const generateTimeLabel = (start: number): string => {
    if (!start && start !== 0) return "Unknow";
    const startHour = 9;

    const hour = Math.floor(startHour + start / 60);
    const displayHour = hour % 12 || 12;
    ...
    return `${displayHour}:${displayMinutes} ${ampmFormat}`;
};
